...
func TestMergeAddsLimenSpecToHumaOpenAPI(t *testing.T) {
	...
	require.Contains(t, schemas, limen.OpenAPIAuthTokensSchema)
	require.Contains(t, schemas, limen.OpenAPIAuthErrorResponseSchema)
	...
	assert.Equal(t, "#/components/schemas/"+limen.OpenAPIAuthTokensSchema, schemas[limen.OpenAPIAuthSessionResponseSchema].Properties["tokens"].Ref)
	require.Contains(t, op.Responses, "401")
	assert.Equal(t, "#/components/schemas/"+limen.OpenAPIAuthErrorResponseSchema, op.Responses["401"].Content["application/json"].Schema.Ref)
